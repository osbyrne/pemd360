import { sqliteTable, AnySQLiteColumn, index, foreignKey, text, integer, uniqueIndex, real, primaryKey } from "drizzle-orm/sqlite-core"
...
(table) => [
	index("categorie_v2_groupe_id_idx").on(table.groupeId),
]);
...
(table) => [
	index("cerfa_mtr_ouvrage_projet_id_idx").on(table.projetId),
]);
...
(table) => [
	index("etablissement_id_societe_id_idx").on(table.idSocieteId),
]);
...
(table) => [
	index("extrapolation_sid_id_idx").on(table.sidId),
]);
...
(table) => [
	index("objets_categorie_id_idx").on(table.categorieId),
]);
...
(table) => [
	index("tags_amiante_sid_id_idx").on(table.sidId),
]);
...
(table) => [
	index("tags_plomb_sid_id_idx").on(table.sidId),
]);
...
(table) => [
	index("tags_termite_sid_id_idx").on(table.sidId),
]);
...
(table) => [
	index("cerfa_diagnostic_projet_id_idx").on(table.projetId),
]);
...
(table) => [
	index("cerfa_diagnostiqueur_projet_id_idx").on(table.projetId),
]);
...
(table) => [
	index("cerfa_operation_projet_id_idx").on(table.projetId),
]);
...
export const userProjet = sqliteTable("user_projet", {
	userId: text("user_id").notNull().references(() => user.id, { onDelete: "cascade" } ),
...
(table) => [
	...
	primaryKey({ columns: [table.userId, table.projetId], name: "user_projet_user_id_projet_id_pk"})
]);
