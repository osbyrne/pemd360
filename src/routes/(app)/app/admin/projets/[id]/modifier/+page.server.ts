...
export const actions: Actions = {
    default: async ({ request, params }) => {
        ...
        const data = {
            ...
            dateDemarrage: formData.get('dateDemarrage') ? new Date(formData.get('dateDemarrage') as string) : new Date(),
        ...
        if (!data.libelle || !data.reference || !data.idEtabId || !data.rue || !data.ville || !data.section || !data.parcelle) {
        ...
        try {
            ...
            return fail(500, { data, message: 'Erreur lors de la mise à jour: ' + (e instanceof Error ? e.message : 'Erreur inconnue'), success: false });
        }
    }
};
