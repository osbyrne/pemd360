...
import { loadEstablishmentEdit, updateEstablishment } from "$lib/server/workflows/administration";

export const load: PageServerLoad = async ({ params, parent }) => {
  await requireAdmin(parent);
  const result = await runServerEffect(loadEstablishmentEdit(params.id));
...
export const actions: Actions = {
  default: async ({ request, params }) => {
    ...
    const result = await runServerEffect(updateEstablishment(params.id, formData));
    ...
    throw redirect(303, "/app/admin/etablissements/" + result.value.updatedId);
  },
};
