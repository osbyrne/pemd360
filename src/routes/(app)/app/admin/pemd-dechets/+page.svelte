...
<div class="p-6 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Tableau Déchets</h1>
            <p class="text-sm text-gray-500 mt-1">Suivi et classification des déchets.</p>
        </div>
    </div>
            ...
            <table class="w-full text-left text-sm text-gray-600">
                <thead class="bg-gray-50 text-xs uppercase font-semibold text-gray-500 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4">Dépose / Déchets</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    {#each displayedList as item}
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {item.deposeDechet ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'}">
                                    {item.deposeDechet || 'Non spécifié'}
                                </span>
                            </td>
                        </tr>
                    {:else}
