...
export const actions: Actions = {
  default: async ({ request, params }) => {
    ...
    const values = {
      ...
      siretSiren: typePersonne === "morale" ? formData.get("siretSiren")?.toString() || null : null,
    ...
    if (existing) {
      await db.update(cerfaDiagnostiqueur).set(values).where(eq(cerfaDiagnostiqueur.projetId, id));
    } else {
