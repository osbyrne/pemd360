<script lang="ts">
    ...
    const filteredDechets = $derived(
        data.dechets.filter(dechet => {
            ...
            if (filterByValorisation === 'valorisable') {
                return dechet.recyclable === 1 || dechet.reutilisation === 1 || 
                       dechet.valorisationMatiere === 1 || dechet.valorisationEnergetique === 1;
    ...
    $effect(() => {
        ...
        searchTerm; filterByValorisation;
        currentPage = 1; 
    });
...
<div class="p-6 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                ...
                <p class="text-xl font-bold text-gray-900 mt-1">{formatLargeNumber(stats.totalMasse)} <span class="text-sm font-normal text-gray-500">kg</span></p>
            </div>
            <div class="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
                <p class="text-xs font-medium text-gray-500 uppercase">Volume total</p>
                <p class="text-xl font-bold text-gray-900 mt-1">{formatLargeNumber(stats.totalVolume)} <span class="text-sm font-normal text-gray-500">m³</span></p>
            </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-4 mb-4 items-end">
            ...
            {#if searchTerm || filterByValorisation !== 'all'}
                <button
                    onclick={() => { searchTerm = ''; filterByValorisation = 'all'; }}
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    <thead class="bg-gray-50 text-gray-600 border-b border-gray-200">
                        <tr>
                            <th class="px-4 py-3 font-semibold whitespace-nowrap min-w-[300px]">Catégorie / Nature</th>
                            ...
                            <th class="px-2 py-3 font-semibold text-center whitespace-nowrap text-xs border-l border-gray-200">
                                Diag.<br/>filières?
                            </th>
                            ...
                            <th class="px-4 py-3 font-semibold text-left whitespace-nowrap border-l border-gray-200">Conditions / Notes</th>
                        </tr>
                    </thead>
                        ...
                        {#each paginatedDechets as dechet}
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-3 align-top text-xs text-gray-600 border-l border-gray-100">
                                    <div class="max-w-[150px] truncate" title={dechet.stockage || dechet.description || ''}>
            ...
            {#if filteredDechets.length > 0}
                <div class="flex items-center justify-between border-t border-gray-200 bg-gray-50 px-4 py-3">
                    <p class="text-sm text-gray-600">
                        Affichage de <span class="font-semibold">{Math.min(filteredDechets.length, (currentPage - 1) * itemsPerPage + 1)}</span>
                        à <span class="font-semibold">{Math.min(filteredDechets.length, currentPage * itemsPerPage)}</span>
