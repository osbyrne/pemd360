...
export const etablissement = sqliteTable(
	...
	{
		id: integer().primaryKey({ autoIncrement: true }).notNull(),
		societeId: integer('id_societe_id')
			.notNull()
	...
	(table) => [index('etablissement_id_societe_id_idx').on(table.societeId)]
);
...
export const tagMail = sqliteTable(
	'tag_mail',
	{
		...
		projetId: text('projet_id_id', { length: 50 })
			.notNull()
			.references(() => projet.id),
	...
	(table) => [index('tag_mail_projet_id_id_idx').on(table.projetId)]
);
...
export const projet = sqliteTable(
	...
	{
		id: text({ length: 50 }).primaryKey().notNull(),
		etablissementId: integer('id_etab_id')
			.notNull()
	...
	(table) => [index('projet_id_etab_id_idx').on(table.etablissementId)]
);
...
export const pemd = sqliteTable(
	'pemd',
	{
		...
		anchorPosition: text('anchor_position').notNull(),
		stemVector: text('stem_vector').notNull(),
		color: text(),
		image: text(),
		surface: real(),
		dimension: text(),
